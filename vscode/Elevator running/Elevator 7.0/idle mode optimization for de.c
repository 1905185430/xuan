...
int main(void)
{
	...
	int f1, f2;
	int upin1,upout1,upin2,upout2,downin1,downout1,downin2,downout2,up1,up2,down1,down2;
	int sum1, sum2;
	int time = 0;
	printf("现在进行电梯的初始化配置\n");
	printf("请输入电梯的容量\n");
	...
	while (usedspace >= 1)
	{
		...
		upin1 = 0;upout1 = 0;
		upin2 = 0;upout2 = 0;
		downin1 = 0;downout1 = 0;
		downin2 = 0;downout2 = 0;
		...
		for (int h = 1; h <= 100; h++)
		{
			if (all[h][5] == 1)
			{
                ...
                upout2 = upout2 + a1;
                downin2 = downin2 + b2;
                downout2 = downout2 + b1;
				//up2 = up2 +  a1 + 3 * a2;//优先送电梯内乘客
				//down2 = down2 + b1 + 3 * b2;
			}
		}//计算上下行系数
        up1 =  3*upin1 + upout1;
        down1 = 3*downin1 + downout1;
        up2 =  3*upin2 + upout2;
        down2 = 3*downin2 + downout2;
        //电梯闲置判断系数v1，v2：0为闲置
        int v1=1,v2=1;
        if(sum1 == 0 && up1 == 0 && down1 == 0)
        {
            v1=0;
        }
        if(sum2 == 0 && downout2==0 )//电梯二优先闲置
        {
            v2=0;
        }
		//if(sum1==0&&sum2==0)
